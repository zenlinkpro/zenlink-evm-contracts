...
contract Stake is Pausable, ReentrancyGuard, AdminUpgradeable {
    ...
    function stake(uint256 amount) public returns (uint256 added_interest) {
        ...
        Helper.safeTransferFrom(STAKED_TOKEN, msg.sender, address(this), amount);
        
        StakeData memory data = StakeDatas[msg.sender];
        
        data.blocknumner = data.blocknumner < START_BLOCK ? START_BLOCK : block.number;
    ...
    function claim() public returns(uint256 claim_reward_amount){
        ...
        uint256 total_reward = IERC20(REWARD_TOKEN).totalSupply();

        claim_reward_amount = total_reward.mul(data.interest) / _total_insterst;
